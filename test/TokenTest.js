...
const { deployTokenFixture, tranferFromTokenFixture } = require("./helpers/TokenFixtures")
...
describe("Token", () => {
    ...
    describe("Sending Tokens", () =>{
        ...
        describe("Success", () => {
            it("transfers a token balance", async () => {
        ...
        describe("Failure", () => {
            it("It rejects insufficient balance", async () => {
           const { token, deployer, receiver } = await loadFixture(deployTokenFixture) 

           const INVALID_AMOUNT = tokens(1000000000)
    ...
    describe("Delegated Token Transfers", () => {
        ...
        describe("Success", async () => {
           ...
           it("Transfers token allowance", async () => {
                const { token, deployer, receiver } = await loadFixture(tranferFromTokenFixture)
                expect(await token.balanceOf(deployer.address)).to.equal(tokens(999900))
                expect(await token.balanceOf(receiver.address)).to.equal(AMOUNT)
           }) 

           it("Resets token balances", async () => {
                const { token, deployer, exchange } = await loadFixture(tranferFromTokenFixture)
                expect(await token.allowance(deployer.address, exchange.address)).to.equal(0)
           }) 

           it("emits an Transfer event", async () => {
            const { token, deployer, receiver, transaction } = await loadFixture(tranferFromTokenFixture) 
        ...
        describe("Failure", () => {
          ...
          const { token, deployer, receiver, exchange } = await loadFixture(tranferFromTokenFixture) 

           const INVALID_AMOUNT = tokens(1000000000)
           const ERROR = "Token: Insufficient Funds"

          await (await token.connect(deployer).approve(exchange.address, INVALID_AMOUNT)).wait()
          await expect(token.connect(deployer).transfer(exchange.address, INVALID_AMOUNT))
          .to.be.revertedWith(ERROR)
          })
